/**
 * Problem Statement:
 * Given an integer array `citations[]`, where `citations[i]` is the number of citations a researcher received for the ith paper,
 * find the H-index.
 *
 * H-Index Definition:
 * - H-Index is the largest value such that the researcher has at least H papers that have been cited at least H times.
 *
 * Examples:
 *
 * Input: citations[] = [3, 0, 5, 3, 0]
 * Output: 3
 * Explanation: There are at least 3 papers (3, 5, 3) with at least 3 citations.
 *
 * Input: citations[] = [5, 1, 2, 4, 1]
 * Output: 2
 * Explanation: There are 3 papers (with citation counts of 5, 2, and 4) that have 2 or more citations.
 * However, the H-Index cannot be 3 because there aren't 3 papers with 3 or more citations.
 *
 * Input: citations[] = [0, 0]
 * Output: 0
 *
 * Constraints:
 * - 1 ≤ citations.size() ≤ 10^6
 * - 0 ≤ citations[i] ≤ 10^6
 */

// Solution:

/**
 * @param {number[]} citations - Array of citation counts for each paper.
 * @returns {number} - H-index of the researcher.
 */
class Solution {
	hIndex(citations) {
		const n = citations.length
		const count = new Array(n + 1).fill(0)

		// Count the number of papers for each citation count.
		for (const citation of citations) {
			if (citation >= n) {
				count[n]++
			} else {
				count[citation]++
			}
		}

		// Iterate from the end to calculate the H-index.
		let total = 0
		for (let i = n; i >= 0; i--) {
			total += count[i]
			if (total >= i) {
				return i
			}
		}

		return 0
	}
}

/**
 * Time Complexity: O(n)
 * - Counting citations is O(n).
 * - Iterating through the array to find the H-index is O(n).
 *
 * Space Complexity: O(n)
 * - The `count` array requires O(n + 1) space.
 */
